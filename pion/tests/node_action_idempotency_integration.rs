...
async fn enqueue_same_key_is_noop() -> anyhow::Result<()> {
    ...
    .await?;
    assert_eq!(id1, id2);
    let rows = PionNodeActionCommand::query_used(&v, valence::use_!("query PionNodeActionCommand in pion/tests/node_action_idempotency_integration.rs; Valence persistence for this feature path; typed store; visible to test harness.")).await?;
